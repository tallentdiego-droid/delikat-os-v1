...
import { getChecklistEngineData, type ChecklistEngineData, type ChecklistRun, type ChecklistTemplate, type ChecklistTemplateItem } from '../../lib/checklists';
import { KnowledgeGapCard, LinkedKnowledgePanel, MetricCard as SharedMetricCard, OSCard, StatusBadge, CoverageBadge, EmptyState } from '../os';
...
function templateSearchText(template: ChecklistTemplate): string {
  return [
    template.title,
...
function templateMatches(template: ChecklistTemplate, query: string): boolean {
  const needle = query.trim().toLowerCase();
  if (!needle) return true;
  return templateSearchText(template).includes(needle);
}

function groupTemplatesByDepartment(templates: ChecklistTemplate[]): Array<{ department: string; templates: ChecklistTemplate[] }> {
  ...
  return Array.from(grouped.entries())
    .map(([department, items]) => ({
      department,
      templates: [...items].sort((a, b) => a.title.localeCompare(b.title)),
    }))
    .sort((a, b) => a.department.localeCompare(b.department));
}

function SharedMetricCardRow({ label, value, helper }: { label: string; value: string | number; helper?: string }): JSX.Element {
  ...
  return (
    <OSCard className={hasGap ? 'checklistItemCard gap' : 'checklistItemCard'}>
      ...
      <p className="previewText">
        {item.description ?? item.processStep?.description ?? item.gapSummary ?? 'Process-step-derived checklist item.'}
      </p>
      <div className="checklistItemLinks">
        {matchedKnowledge ? (
          <span>Knowledge: {matchedKnowledge.title}</span>
        ) : (
          <span>No required knowledge linked yet.</span>
        )}
...
function ChecklistTemplateDetail({
  ...
  const selectedRuns = template.runs.slice().sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    ...
    <div className="detailStack checklistDetail">
      <section className="detailSection">
        ...
        <div className="summaryGrid">
          <SharedMetricCardRow label="Linked process" value={template.process?.name ?? 'None'} helper={template.process?.code ?? undefined} />
          <SharedMetricCardRow label="Department" value={valueOrDefault(template.department?.title)} helper={template.department?.code ?? undefined} />
          <SharedMetricCardRow label="Role" value={valueOrDefault(template.role?.title)} helper={template.role?.code ?? undefined} />
          <SharedMetricCardRow label="Area" value={valueOrDefault(template.area?.title)} helper={template.area?.code ?? undefined} />
          <SharedMetricCardRow label="Items" value={template.itemCount} helper={`${template.linkedKnowledgeCount} linked knowledge items`} />
          <SharedMetricCardRow label="Runs" value={template.runCount} helper={`${template.openRunCount} open runs`} />
        </div>
      </section>
        ...
        <div className="checklistItemList">
          {template.items.length > 0 ? (
            ...
            <EmptyState
              ...
              description="Template items will appear once the linked process steps are seeded."
            />
          )}
        </div>
      </section>
...
export function ChecklistsModule({ onOpenKnowledgeBase }: ChecklistsModuleProps = {}): JSX.Element {
  ...
  useEffect(() => {
    ...
    getChecklistEngineData()
      .then((nextData) => {
        if (!isMounted) return;
        setData(nextData);
        setSelectedTemplateId((current) => current ?? nextData.templates[0]?.id ?? null);
      })
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return (data?.templates ?? []).filter((template) => templateMatches(template, query));
  }, [data, filters.query]);
  ...
  const selectedTemplate = useMemo(() => filteredTemplates.find((template) => template.id === selectedTemplateId) ?? null, [filteredTemplates, selectedTemplateId]);
  const groupedTemplates = useMemo(() => groupTemplatesByDepartment(filteredTemplates), [filteredTemplates]);
    ...
    <section className="pageStack">
      <div className="sectionHeader">
        ...
        <div className="resultsMeta">
          {data ? (
            <span>
              {data.stats.totalTemplates} templates, {data.stats.totalItems} items
            </span>
          ) : (
            <span>Loading live checklist data</span>
          )}
        </div>
      </div>
      ...
      <div className="metricGrid checklistSummaryGrid">
        <SharedMetricCardRow label="Templates" value={data?.stats.totalTemplates ?? '...'} helper="Starter templates from seeded processes" />
        <SharedMetricCardRow label="Checklist items" value={data?.stats.totalItems ?? '...'} helper="Derived from live process steps" />
        <SharedMetricCardRow label="Templates with gaps" value={data?.stats.templatesWithGaps ?? '...'} helper="Need knowledge coverage" />
        <SharedMetricCardRow label="Missing coverage" value={data?.stats.itemsMissingCoverage ?? '...'} helper="Checklist items without approved knowledge" />
      </div>
        ...
        <section className="listPanel checklistListPanel">
          {groupedTemplates.length > 0 ? (
              ...
              <section className="checklistGroup" key={group.department}>
                ...
                <div className="checklistTemplateList">
                  {group.templates.map((template) => (
                    <TemplateCard key={template.id} template={template} selected={template.id === selectedTemplateId} onSelect={setSelectedTemplateId} />
                  ))}
                </div>
              </section>
            ))
            ...
            <EmptyState
              ...
              description="Once starter processes are seeded in Supabase, checklist templates will appear here."
            />
          )}
        </section>
        ...
        <section className="detailPanel checklistDetailPanel">
          {selectedTemplate ? (
            ...
            <EmptyState
              ...
              description="Choose a template to inspect its steps, linked knowledge, and coverage gaps."
            />
          )}
        </section>
      </div>
