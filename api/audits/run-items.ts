...
export default async function handler(req: any, res: any): Promise<void> {
  ...
  try {
    ...
    const auditRunItemId = typeof payload.auditRunItemId === 'string' ? payload.auditRunItemId : '';
    const status = typeof payload.status === 'string' ? payload.status : '';
    const score = typeof payload.score === 'number' && Number.isFinite(payload.score) ? payload.score : null;
    const notes = typeof payload.notes === 'string' ? payload.notes : null;

    if (!auditRunItemId) {
      res.status(400).json({ error: 'An audit run item id is required.' });
      return;
    }
    ...
    const { data: updatedItem, error: updateError } = await client
      ...
      .eq('organization_id', ORGANIZATION_ID)
      .eq('id', auditRunItemId)
      .select('id,audit_run_id')
      .single();

    if (updateError) throw updateError;
    if (!updatedItem) {
      res.status(404).json({ error: 'Audit run item not found.' });
      return;
    }

    await recalculateRun(client, updatedItem.audit_run_id);

    res.status(200).json({
      runId: updatedItem.audit_run_id,
      itemId: updatedItem.id,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Audit item update failed.';
